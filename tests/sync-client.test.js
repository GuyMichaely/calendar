...
import { CalendarSyncError, exchangeCalendarSnapshot, syncLatestCalendarDocument } from "../sync/client.js";
...
test("sync client exchanges Automerge bytes through ordinary fetch", async () => {
  ...
  const remote = await exchangeCalendarSnapshot(local, {
...
test("sync client exposes HTTP authentication failures", async () => {
  ...
  await assert.rejects(
    exchangeCalendarSnapshot(local, {
      endpoint: "https://sync.example/sync",
      fetch: handlerFetch(handler),
    }),
...
test("sync merges the response into the latest local document so in-flight edits survive", async () => {
  ...
  const syncing = syncLatestCalendarDocument(() => current, {
    endpoint: "https://sync.example/sync",
    fetch,
  });
  await reachedGate;
  ...
  const item = materializeItem(merged, "task-1");
