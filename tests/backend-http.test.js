...
test("untrusted Origin requests are rejected before auth or sync handlers run", async () => {
  let calls = 0;
  const backend = createCalendarBackend({
    authHandler: async () => { calls += 1; return new Response("auth"); },
    syncHandler: async () => { calls += 1; return new Response("sync"); },
    allowedOrigins: ["https://app.example"],
  });

  for (const request of [
    new Request("https://sync.example/auth/me", { headers: { origin: "https://evil.example" } }),
    new Request("https://sync.example/sync", { method: "POST", headers: { origin: "https://evil.example" } }),
  ]) {
...
test("requests without Origin remain available to direct navigation and server clients", async () => {
  const backend = createCalendarBackend({
  ...
  const response = await backend(new Request("https://sync.example/auth/me"));
