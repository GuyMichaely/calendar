...
test("Solid persistence uploads attachment bytes before storing metadata-only Automerge state", async () => {
  ...
  assert.equal(db.objectStoreNames.contains("attachments"), false);
  db.close();

  const remoteDocument = addTag(forkCalendarDocument(baseDocument), initial.id, "remote");
  await storage.putItem({ ...items[0], title: "Plan pool swim", tags: [...items[0].tags, "local"], updatedAt: "2026-09-04T13:00:00.000Z" }, items[0]);
...
test("a stale Solid item save applies only the user's delta to the latest merged document", async () => {
  ...
  remoteDocument = patchItem(remoteDocument, id, { deadline: "2026-10-02T17:00:00.000Z", notes: "remote notes" });
  remoteDocument = addTag(remoteDocument, id, "remote");
  remoteDocument = addHistoryEntry(remoteDocument, id, { at: "2026-09-04T12:30:00.000Z", type: "remote-edit" });
  await storage.mergeSyncSnapshot(saveCalendarDocument(remoteDocument));
  await storage.putItem({ ...baseline, tags: [...baseline.tags, "local"], updatedAt: "2026-09-04T13:00:00.000Z" }, baseline);
...
test("a stale editor text change is made at its captured heads so concurrent remote text survives", async () => {
  ...
  const baseline = (await storage.listItems()).find((item) => item.id === id);
  const baseDocument = loadCalendarDocument(await storage.readSyncSnapshot());
  const remoteDocument = updateItemText(forkCalendarDocument(baseDocument), id, "notes", "hello world!");
  await storage.mergeSyncSnapshot(saveCalendarDocument(remoteDocument));
  await storage.putItem({ ...baseline, notes: "hello brave world", updatedAt: "2026-09-04T13:30:00.000Z" }, baseline);
...
test("kind conversion removes obsolete source-kind fields without overwriting untouched shared remote edits", async () => {
  ...
  const baseline = (await storage.listItems()).find((item) => item.id === id);
  const baseDocument = loadCalendarDocument(await storage.readSyncSnapshot());
  let remoteDocument = patchItem(forkCalendarDocument(baseDocument), id, { deadline: "2026-10-05T17:00:00.000Z" });
  remoteDocument = addTag(remoteDocument, id, "remote");
  await storage.mergeSyncSnapshot(saveCalendarDocument(remoteDocument));
  await storage.putItem({ id, kind: "event", title: baseline.title, notes: baseline.notes, tags: baseline.tags, attachments: baseline.attachments, start: "2026-09-20T13:00:00.000Z", end: "2026-09-20T14:00:00.000Z", createdAt: baseline.createdAt, updatedAt: "2026-09-04T14:00:00.000Z" }, baseline);
  const current = (await storage.listItems()).find((item) => item.id === id);
