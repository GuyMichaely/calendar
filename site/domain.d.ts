...
export type Actionability = { actionable: boolean; reason: string };
...
export function nextActionableStart(task: Task, now?: Date, options?: { respectSleep?: boolean }): Date | null;
export function isWaitingForOpportunity(task: Task, now?: Date): boolean;
export function tomorrowMidnight(now?: Date): Date;
export function availabilityStartForDate(task: Task, date: DateInput, now?: Date, options?: { respectSleep?: boolean }): Date | null;
