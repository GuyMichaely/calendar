...
function actionTooltip(label, shortcutAction, shortcutHints) {
  const hint = shortcutHints?.[shortcutAction];
  return hint ? `${label} (${hint})` : label;
}

function iconActionButton(action, shortcutAction, label, icon, shortcutHints) {
  const tooltip = actionTooltip(label, shortcutAction, shortcutHints);
  return `<button type="button" class="task-action-icon" data-action="${action}" aria-label="${escapeHtml(tooltip)}" title="${escapeHtml(tooltip)}">${icon}</button>`;
}

function taskCard(task, now, upcomingAt = null, showAvailability = false, shortcutHints = {}) {
  ...
  article.innerHTML = `
    ...
    <div class="task-actions">
      ...
      ${!closed && !sleep.sleeping ? iconActionButton("sleep-tomorrow", "sleepTomorrow", "Sleep until tomorrow", ACTION_ICONS.sleepTomorrow, shortcutHints) : ""}
      ${!closed && !sleep.sleeping ? iconActionButton("sleep-indefinite", "sleepIndefinite", "Sleep indefinitely", ACTION_ICONS.sleepIndefinite, shortcutHints) : ""}
      ${!closed && !sleep.sleeping ? iconActionButton("sleep-custom", "customSleep", "Custom sleep", ACTION_ICONS.customSleep, shortcutHints) : ""}
...
function taskSection(definition, rows, sleepingRows, model, now) {
  ...
  if (!rows.length) {
    ...
    for (const row of rows) {
      list.append(taskCard(row.task, now, row.upcomingAt, definition.id === "upcoming", model.shortcutHints));
    }
  }
  body.append(list);
  ...
  if (definition.id === "upcoming" && sleepingRows.length) {
    ...
    for (const row of sleepingRows) sleepingList.append(taskCard(row.task, now, row.upcomingAt, true, model.shortcutHints));
