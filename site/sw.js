...
const IS_PREVIEW_SCOPE = /\/(?:vanilla|framework|solid|svelte)\/$/.test(SCOPE_PATH);
const LEGACY_CACHES = IS_PREVIEW_SCOPE ? new Set() : new Set(["calendar-shell-v12"]);
const PREVIEW_PATHS = IS_PREVIEW_SCOPE
  ? []
  : ["vanilla/", "framework/", "solid/", "svelte/"].map((segment) => new URL(segment, self.registration.scope).pathname);
