...
Use the + button beside a task or Add subtask in its editor to create a child. Drag the card body or title to move an existing task. On touch screens, hold briefly before dragging; ordinary swipes still scroll. The two-line grip also supports immediate dragging. A floating copy follows the pointer while the original stays dimmed to mark its previous position; Escape cancels the drag. Drop near the top/bottom of a card to reorder siblings, in its center to make it a child, or in the top-level drop area to make it independent. On a focused handle, Up/Down reorders and Left makes the task top-level. Name a new task before adding its children.
