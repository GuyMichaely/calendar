...
Use the + button beside a task or Add subtask in its editor to create a child. Drag the card body or title to move an existing task. On touch screens, hold briefly before dragging; ordinary swipes still scroll. The two-line grip also supports immediate dragging. A floating copy follows the pointer while the original stays dimmed to mark its previous position; Escape cancels the drag. Other rows move aside to show a card-sized gap at the destination. Drop near the top/bottom of a card to reorder siblings, or in its center to make it a child (shown by an indented gap beneath its subtree). Dropping beside a top-level task makes the dragged task independent. A separate top-level drop area appears only while dragging a subtask, so it can still be detached when filtering hides other tasks. On a focused handle, Up/Down reorders and Left makes the task top-level. Name a new task before adding its children. The Animations setting controls the sliding movement; the placement gap remains visible when animations are disabled.
