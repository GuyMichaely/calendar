import { actionability, isSleeping, nextActionableStart, sleepInfo, toDate } from "../../site/domain.js";
...
export function planTasks(tasks: Task[], now: Date, respectSleep: boolean, sort: TaskSort, horizon: Date | null) {
  ...
  const compare = (a: Task, b: Task): number => {
    if (sort === "manual") return (a.sortOrder ?? Infinity) - (b.sortOrder ?? Infinity) || a.title.localeCompare(b.title);
    const primary = dateKey(a) - dateKey(b);
    return primary || (toDate(a.deadline)?.getTime() ?? Infinity) - (toDate(b.deadline)?.getTime() ?? Infinity) || a.title.localeCompare(b.title);
  };
